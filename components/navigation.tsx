            ...
            <div className="hidden lg:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`text-sm font-medium tracking-wide transition-colors duration-200 hover:text-primary relative group ${
                    pathname === link.href ? 'text-primary' : 'text-white/80'
                  }`}
                >
                  {link.label}
                  <span className={`absolute -bottom-1 left-0 w-full h-px bg-primary transition-transform duration-300 origin-left ${
